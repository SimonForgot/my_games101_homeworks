...
void rst::rasterizer::rasterize_triangle(const Triangle& t, const std::array<Eigen::Vector3f, 3>& view_pos) 
{
    ...
    auto v = t.toVector4();
    auto min_bound=Vector2f(std::min(t.v[0].x(),std::min(t.v[1].x(),t.v[2].x())),
    std::min(t.v[0].y(),std::min(t.v[1].y(),t.v[2].y())));
    auto max_bound=Vector2f(std::max(t.v[0].x(),std::max(t.v[1].x(),t.v[2].x())),
    std::max(t.v[0].y(),std::max(t.v[1].y(),t.v[2].y())));
    ...
    for(int j=min_bound.y();j<max_bound.y();j++)
    {
        if(insideTriangle(i, j, t.v))
        {
            ...
            if(zp<depth_buf[get_index(i,j)])
            {
                ...
                set_pixel(Vector2i(width-i,height-j),pixel_color);
            }
        }
    }
}
